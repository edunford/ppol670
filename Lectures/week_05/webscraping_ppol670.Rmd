...
- **Discussion of Loops and Functions**
...
API Set up: `/api/data/nibrs/{offense}/victim/national/{variable}`

Make call to the API
```{r}
fbi_api <- 'https://api.usa.gov/crime/fbi/sapi/api/data/nibrs/homicide/victim/national/age'
get_fbi_dat <- GET(fbi_api,query=list(api_key=my_api_key))
fbi_dat <- content(get_fbi_dat)

# Tap into the results...
as_tibble(fbi_dat$results[[1]])
```
...
```{r}
total_entries <- length(fbi_dat$results) 
total_entries
```
...
for(i in 1:total_entries){
  tmp <- as_tibble(fbi_dat$results[[i]])  #<<
...
homicide_data_clean <-
  homicide_data %>%  
  select(-offense_name) %>%   # Drop category (only homicides)
  rename(year = data_year) %>%  # Rename year var
  gather(age_range,count,-year) %>%  # Go from wide to long data
  mutate(age_range = str_remove(age_range,'range_')) %>%  # remove from age categories
