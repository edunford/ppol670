...
pol %>%
  ...
  labs(x="Polity Score",
       ...
       caption = "Source: World Bank and Systemic Peace"
       ) +
  theme(legend.position = "bottom")
```
]
...
```{r}
pol2 <-
  read_csv(here::here("Lectures/week_05/R-code/dem-gdp-data.csv")) %>%
...
pol2 %>%
  ...
  labs(y="Polity",
       ...
       caption = paste0("* Only South American countries with complete data from 1850 to 2016 are shown",
                        "\nSource: World Bank and Systemic Peace")
       ) 
```
]
...
Let's now work through a live example using the [gapminder](https://www.gapminder.org/data/) data.
...
Let's build up a plot together, I'll then post the code on Slack.
