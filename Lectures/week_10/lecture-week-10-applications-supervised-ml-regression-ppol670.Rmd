...
```{r}
bus1 <- tibble(age=c(5,20,21,14,17,23,26))
bus2 <- tibble(age=c(23,28,37,51,18,30,48))
```
...
for(i in 1:100){
  boot_samp1 = bus1 %>% sample_n(4,replace = T)
  boot_samp2 = bus2 %>% sample_n(4,replace = T)
...
ggplot(aes(x=means)) + 
  ...
  geom_vline(xintercept = -15.57143,color="red",size=1) # actual mean#<< 
```
