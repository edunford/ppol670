---
title: "PPOL670 | Introduction to Data Science | Week 11"
subtitle: "Walkthrough using classification methods with `caret`"
output: 
  html_notebook:
    df_print: paged
    theme: united
...
```{r Setup, include=F}
knitr::opts_chunk$set(warning = F,error = F,message = F,comment = F,cache=T)
```
...
```{r,fig.height=6,fig.width=6,fig.height=10}
train_data %>% 
  select_if(is.factor) %>% 
  gather(var,val) %>% 
  ggplot(aes(val)) +
