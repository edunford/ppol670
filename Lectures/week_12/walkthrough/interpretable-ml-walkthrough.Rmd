...
nohs_pdp <- partial(mod_rf_selected, pred.var = "educ_Less.than.HS", plot = TRUE,prob=T,
                   plot.engine = "ggplot2")
...
```{r,fig.align="center",fig.height=10,fig.width=10}
# PDP for age
age_ice <- partial(mod_rf_selected, pred.var = "age", plot = TRUE,prob=T,
                   ice=T,center=T,alpha=.05,
                   grid.resolution = 20, # choosing less points makes it run quicker
                   plot.engine = "ggplot2")

# PDP for wage
wage_ice <- partial(mod_rf_selected, pred.var = "wage", plot = TRUE,prob=T,
                    ice=T,center=T,alpha=.05,
                    grid.resolution = 20, # choosing less points makes it run quicker
                   plot.engine = "ggplot2")

# PDP for Married (note that this is a dummy var, so only can take on one of two values)
mar_ice <- partial(mod_rf_selected, pred.var = "mar_Married", plot = TRUE,prob=T,
                   ice=T,center=T,alpha=.05,
                   plot.engine = "ggplot2")

# PDP for Not a citizen (note that this is a dummy var, so only can take on one of two values)
cit_ice <- partial(mod_rf_selected, pred.var = "cit_Non.citizen", plot = TRUE,prob=T,
                   ice=T,center=T,alpha=.05,
                   plot.engine = "ggplot2")

# PDP for No high school degree (note that this is a dummy var, so only can take on one of two values)
nohs_ice <- partial(mod_rf_selected, pred.var = "educ_Less.than.HS", plot = TRUE,prob=T,
                    ice=T,center=T,alpha=.05,
                   plot.engine = "ggplot2")
